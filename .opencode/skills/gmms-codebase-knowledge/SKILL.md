...
- **Payment UI** (`Feature/Payment/`): fully redesigned to the design system — page header + subtitle, amber stats card (Total Payments), search + `MudDateRangePicker` toolbar (FromDate/ToDate wired to `GetPaymentListAsync`), skeleton rows, error + Try Again, outlined table (no duplicate # / Payment ID columns), `gmm-pay__*` scoped CSS. `PaymentCreate`: `MudSelect` pickers + "Selected Membership" context card (member code chip, plan, dates, status chip) + skeleton/loadFailed states. `PaymentDetail`: receipt header (big green amount + colored status chip) + Timestamps audit section.
