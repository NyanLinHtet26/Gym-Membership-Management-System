...
- Routes: `/` Dashboard (home), `/login`, `/change-password` (AuthLayout); `/member-list`, `/membership-list?memberId=`, `/membership-list-all`, `/membershipplan-list`, `/paymentmethod-list`, `/payment-list`, `/user-list` (MainLayout). Counter/Weather are template stubs (Home.razor deleted).
...
- Dialogs per feature: Create/Edit/Delete/Detail as `MudDialog` components, dialog closes with `DialogResult.Ok(true)` triggering list reload. Raw HTML `<select>` (form-select) used for Member/Plan/PaymentMethod pickers in MembershipCreate/PaymentCreate.
